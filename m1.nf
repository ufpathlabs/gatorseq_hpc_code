...
output:
	...
	bwa mem    -t ${params.bwa_map_sort.threads}  -M ${params.human_ref_bwa}  ${r1} ${r2} 2> ${s}${params.bwa_map_sort.log} | sambamba view --sam-input  --compression-level 0   --nthreads 1  --format bam   --output-filename ${s}.bwa_map.bam /dev/stdin 2>> ${s}${params.bwa_map_sort.log}
		
	sambamba sort --nthreads=${params.bwa_map_sort.threads} --memory-limit=${params.bwa_map_sort.memory}  --tmpdir= tempdir   --out=${s}.bwa_map_sort.bam   --compression-level=0    ${s}.bwa_map.bam 2>> ${s}${params.bwa_map_sort.log}
